...
(define (create-linalg-element name type val)
  (list (cons 'name name) (cons 'type type) (cons 'val val)))


(define (linalg-element? exp)
  (and (alist? exp)
    (equal? (map car exp) '(name type val))))

(define (get-name exp)
  (let ((res (assq 'name exp)))
    (if res
        (cdr res)
        #f)))

(define (get-type exp)
  (let ((res (assq 'type exp)))
    (if res
        (cdr res)
        #f)))

(define (get-val exp)
  (let ((res (assq 'val exp)))
    (if res
        (cdr res)
        #f)))
...
;;; Variables
(define matrix-tag (cons 'type 'matrix))
(define vector-tag (cons 'type vector))

(define (variable? exp) (symbol? exp))

(define (same-variable? var1 var2) (eq? var1 var2))  ;; Nice abstraction

; (define (matrix-tagged? exp)
;   (tagged-list? exp matrix-tag))

; (define (vector-tagged? exp)
;   (tagged-list?  exp vector-tag))

; (define (save-name? exp)
;   (or (matrix-tagged? exp) 
;       (vector-tagged? exp)))
