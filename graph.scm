...
(define (append-node-data! node msg)
  (let ((cur-message (node-data node)))
    (set-node-data! node (string-append cur-message msg))))
...
(define (parent child)
  (let ((p (map edge-src-node (get-incoming-edges child))))
    (if (null? p)
        '()
        (car p))))

(define (children parent)
  (map edge-dest-node (get-outgoing-edges parent)))
